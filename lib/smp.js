...
class StakingManagementPolicy {
    ...
    submitTransaction(input, retryAttempts = 5) {
        ...
        return new Promise((resolve, reject) => {
            Async.retry(
                {
                    times: retryAttempts,
                    interval: count => 500 * Math.pow(2, count) // 1s, 2s, 4s ...
                },
                task,
                (err, result) => _.isNil(err) ? resolve(result) : reject(err));
        });
    }
    ...
    async sendStakeViaWallet(dbEntry, sendOnce, maxFactor, retryAttempts) {
        ...
        await this.sendStakeImpl(dbEntry, srcAddr, dstAddr, stake, maxFactor, retryAttempts);
    }
    ...
    async sendStakeViaDePool(dbEntry, maxFactor, retryAttempts) {
        ...
        const { addr: dstAddr, eventAnticipationTimeout = 60000 } = this.policy.funding;

        // Extract a proxy address from depool events...
        const matchStakeSigningRequested = _.matches({
            body: {
        ...
        let eventBody1 = await lookForDePoolEvent(
            this.client,
            this.policy.funding.addr,
            dbEntry.id,
            matchStakeSigningRequested);

        if (_.isNil(_.get(eventBody1, 'value.proxy'))) {
            debug(`INFO: 'StakeSigningRequested' event not found - let's wait for it...`);

            await this.sendTicktock();

            eventBody1 = await waitForDePoolEvent(
                this.client,
                this.policy.funding.addr,
                eventAnticipationTimeout,
                matchStakeSigningRequested);
        }

        const srcAddr = _.get(eventBody1, 'value.proxy');
        ...
        await this.sendStakeImpl(dbEntry, srcAddr, dstAddr, 1, maxFactor, retryAttempts);

        // Ensure that stake either accepted or rejected...
        const matchStakeProcessingFinished = _.conforms({
            body: ({ name }) => _.some(
    ...
    async sendStake(sendOnce = true, maxFactor = 3, retryAttempts = 5) {
        ...
        if (activeElectionId === 0) {
            ...
            if (this.flags.postElectionsTicktockIsSent) return;
...
async function lookForDePoolEvent(client, depoolAddress, activeElectionId, isWhatWeAreLookingFor) {
    const { result } = await client.net.query_collection({
        collection: 'messages',
        filter: {
            src: { eq: depoolAddress },
            msg_type: { eq: 2 },
            created_at: { gt: activeElectionId }
        },
        result: 'body'
    });
