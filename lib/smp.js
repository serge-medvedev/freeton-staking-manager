...
class StakingManagementPolicy {
    ...
    async lookForStakeSigningRequest(activeElectionId) {
        ...
        const { result } = await this.client.net.query_collection({
            ...
            filter: {
                src: { eq: depoolAddr },
                msg_type: { eq: 2 },
                created_at: { ge: Math.floor(Date.now() / 1000) - 86400 }
            },
            result: 'body'
        });
        ...
        const isWhatWeAreLookingFor = _.matches({
            body: {
                name: 'StakeSigningRequested',
                value: {
                    electionId: activeElectionId.toString()
                }
            }
        });
    ...
    waitForStakeSigningRequest(activeElectionId, timeout) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const isWhatWeAreWaitingFor = _.matches({
                body: {
                    name: 'StakeSigningRequested',
                    value: {
                        electionId: activeElectionId.toString()
