...
class StakingManager {
    constructor(client, datastore, electorAddr) {
        this.client = client;
    ...
    static async create() {
        ...
        const electorAddr = `-1:${await getConfig(1, client)}`;

        return new StakingManager(client, datastore, electorAddr);
    }
    ...
    async recoverStake(sendAttempts = 10) {
        await this.recoverStakeImpl(sendAttempts);
    }
    ...
    async recoverStakeImpl(sendAttempts) {
    ...
    async getLatestStakeAndWeight() {
        const key = _
            ...
            .value();
        const calcWeight = p => {
            return _.chain(p).get('list').find({ adnl_addr: key }).get('weight').parseInt().value() /
                   _.chain(p).get('total_weight').parseInt().value();
        }

        let weight = calcWeight(await getConfig(34, this.client));

        if (weight === 0) {
            weight = calcWeight(await getConfig(36, this.client));
        }

        const pastElectionsInfo = await this.client.contracts.runGet({
            address: this.electorAddr,
            functionName: 'past_elections'
        });
        const totalStake = _.chain(pastElectionsInfo).get('output.0.0.5').parseInt().value();
    ...
    async getPastElectionIds() {
        ...
        return _.chain(result).get('output.0').map(_.parseInt).value();
    }
}
