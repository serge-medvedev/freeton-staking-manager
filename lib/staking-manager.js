...
class StakingManager {
    ...
    getConfig(id) {
        if (_.some([34, 36], p => p === id)) {
            return this.tonConfig.get(id);
        }
        else {
            return this.getConfigMemoized(id);
        }
    }
    ...
    async getLatestStakeAndWeight() {
        const key = _
            ...
            .value();
        const calcWeight = p => {
            return _.chain(p).get('list').find({ adnl_addr: key }).get('weight').defaultTo(0).parseInt().value() /
                   _.chain(p).get('total_weight').parseInt().value();
        }

        let weight = calcWeight(await this.getConfig(34));

        if (weight === 0) {
            weight = calcWeight(await this.getConfig(36));
        }
