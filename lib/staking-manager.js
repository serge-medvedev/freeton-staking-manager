...
class StakingManager {
    ...
    async recoverStake(electorAddr, sendAttempts) {
    ...
    async sendStake(electorAddr, activeElectionId, maxFactor, sendAttempts) {
        ...
        for (let n = 1; n <= sendAttempts; ++n) {
            ...
            if (_.get(result, 'transaction.action.success')) {
                await this.datastore.setElectionsInfo({
                    id: activeElectionId,
                    stake,
                    adnlKey: electionADNLKey,
                    publicKey
                });
    ...
    async runOnceImpl(maxFactor, sendAttempts) {
        ...
        if (activeElectionId === 0) {
            ...
            await this.recoverStake(electorAddr, sendAttempts);
        }
        else {
            ...
            await this.sendStake(electorAddr, activeElectionId, maxFactor, sendAttempts);
        }
    }
