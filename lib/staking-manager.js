...
class StakingManager {
    ...
    async sendStakeViaDePool(sendOnce = true, maxFactor = 3, sendAttempts = 10) {
        ...
        const srcAddr = await this.waitForStakeSigningRequest(activeElectionId);
        const dstAddr = depool.addr;

        if (_.isNil(srcAddr)) {
            throw new Error('Unable to detect relevant proxy address from DePool events');
        }
    ...
    waitForStakeSigningRequest(activeElectionId) {
    ...
    async getActiveDePoolProxyAddr(activeElectionId) {
        const results = await this.client.queries.messages.query(
            {
                src: { eq: depool.addr },
                msg_type: { eq: 2 },
                created_at: { ge: moment().subtract(1, 'week').unix() }
            },
            'body'
        );
