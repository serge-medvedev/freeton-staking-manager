...
const { execFift, execValidatorEngineConsole, execGenerateRandomId } = require('./ton-tools');
...
async function getNewKeyPair() {
    ...
    const secretKeyFileContentsBase64 = await fs.readFile(path, { encoding: 'base64' });
    ...
    async restoreKeys() {
        ...
        for (const id of ids) {
            ...
            for (const secret of info.secrets) {
                ...
                await fs.writeFile(path, secret, { encoding: 'base64' });
