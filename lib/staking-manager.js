...
class StakingManager {
    ...
    async getLatestStakeAndWeight() {
        ...
        const calcWeight = adnl_addr => {
            return _.chain(p34).get('list').find({ adnl_addr }).get('weight').parseInt().value() /
                   _.chain(p34).get('total_weight').parseInt().value();
        }
        const weight = calcWeight(curKey) || calcWeight(prevKey);
